...
const Skills = () => {
  return (
    <section id='skills' className='py-24 pb-[12vw]  md:px-[7vw] lg:px-[20vw] font-sans bg-skills-gradient '>
      ...
      <div className='flex flex-wrap px-5 gap-1 lg:gap-5 py-10 justify-between'>{SkillsInfo.map((category) => (<div key={category.title} className='bg-gray-900 backdrop-blur-md px-6 sm:px-10 py-8 sm:py-6 mb-10 w-full sm:w-[48%] rounded-2xl border border-purple-300 hover:border-purple-600 shadow-[0_0_20px_1px_rgba(130,69,236,0.3)]'>
      <Tilt>
      <h3 className=' text-2xl sm:text-3xl text-gray-400 mb-4 text-center'>{category.title}</h3>
        ...
        <div className=' grid grid-cols-2 lg:grid-cols-2  md:grid-cols-2 sm:grid-cols-2  gap-4 w-full'>{category.skills.map((skill) =>
          <div
            key={skill.name}
            className='flex md:text-2xl items-center justify-center space-x-2 bg-transparent border-2 border-gray-700 hover:border-purple-600  rounded-3xl py-4 px-4 sm:py-2 sm:px-2 text-center'
          >
